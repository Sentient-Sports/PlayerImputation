...
Data Extracted from the Kloppy package. Minor changes made such as removal as score as this is not provided.
Some minor changes to make to XML file received by BePro tracking:
1. Metadata tag -> MetaData tag
2. Seperator tag -> seperator tag
3. Velocity channel must be added as a seperate channel
4. Frames per second must be added
...
from getTracking.models import *
...
def load_metadata(
    ...
    #_team_map = {
    #    Ground.HOME: team_elm.attrib['id'],#score_elm.attrib["idLocalTeam"],
    ...
    _team_map = { 
        Ground.HOME: list(_team_name_map.keys())[0],
        Ground.AWAY: list(_team_name_map.keys())[1]        
    }
    ...
    player_channels = [
        ...
        if player_channel_elm.attrib["channelId"] != 'velocity'
    ]

    #Default FPS for BePro data
    frame_rate = 30 #int(metadata.find("GlobalConfig").find("FrameRate"))
